...
$query_hash = mysql_query("SELECT * FROM users where username = '$username'");

// if exists, checks if the entered password matches the hashed password
while ($row = mysql_fetch_array($query_hash)){
...
$query_password = mysql_query("SELECT * FROM users where username = '$username' AND password='$checked_password'");
if(mysql_num_rows($query_password) > 0){
...
{	    
    $query_wrongpassword = mysql_query("SELECT * FROM users where username = '$username'");
    if(mysql_num_rows($query_wrongpassword) > 0)
    {
    ...
    else
    {
        ...
        if ($username != "" && $password != ""){
            mysql_query($query_newuser, $connect) or die("error inserting username and password" . mysql_error());
